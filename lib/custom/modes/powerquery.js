...
CodeMirror.defineMode('powerquery', function (conf, parserConf) {
  ...
  function indent(stream, state, type) {
    type = type || 'powerquery'
    var indentUnit = 0
    if (type === 'powerquery') {
      if (state.scopes[0].type !== 'powerquery') {
        state.scopes[0].offset = stream.indentation()
        return
      }
      for (var i = 0; i < state.scopes.length; ++i) {
        if (state.scopes[i].type === 'powerquery') {
  ...
  function tokenLexer(stream, state) {
    ...
    if (state.dedent > 0 && stream.eol() && state.scopes[0].type == 'powerquery') {
  ...
  var external = {
    startState: function (basecolumn) {
      return {
        tokenize: tokenBase,
        scopes: [{ offset: basecolumn || 0, type: 'powerquery' }],
        lastStyle: null,
        lastToken: null,
        lambda: false,
        dedent: 0,
